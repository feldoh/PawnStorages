...
public class FloatMenuOptionProvider_Arrest : FloatMenuOptionProvider
{
        ...
        FloatMenuContext context)
    {
        ...
        if (context.FirstSelectedPawn.InSameExtraFaction(clickedPawn, ExtraFactionType.HomeFaction) ||
        ...
        foreach (CompAssignableToPawn_PawnStorage storage in WorkGiver_Warden_TakeToStorage.GetPossibleStorages(clickedPawn).GroupBy(s => s.parent.def)
                     .Select(sGroup => sGroup.FirstOrDefault()))
        {
            ...
            yield return FloatMenuUtility.DecoratePrioritizedTask(
                new FloatMenuOption(
                    (notArresting ? "PS_TakeToStorageFloatMenu" : "PS_CaptureToStorageFloatMenu").Translate((NamedArgument) clickedPawn.LabelCap,
                    ...
                    () =>
                    {
                        ...
                        Job job = JobMaker.MakeJob(notArresting ? PS_DefOf.PS_TakeToPawnStorage : PS_DefOf.PS_CaptureInPawnStorage,
                            (LocalTargetInfo) (Thing) clickedPawn, (LocalTargetInfo) (Thing) building);
                        job.count = 1;
                    ...
                    revalidateClickTarget: clickedPawn), clickedPawn,
                (LocalTargetInfo) (Thing) clickedPawn);
        }

        foreach (CompPawnStorage comp in clickedPawn.inventory.GetDirectlyHeldThings()
        ...
        if (!anyStorage)
        {
            yield return new FloatMenuOption(
                "PS_NoPrisonerStorage".Translate((NamedArgument) clickedPawn.Label),
                null);
        }
    }
}
